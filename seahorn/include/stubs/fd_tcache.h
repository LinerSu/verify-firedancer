...
#ifdef __SEAHORN__
   /* Explicit storage for ring[] then map[]: */
   ulong *ring; ulong *map;
#endif
...
#ifdef __SEAHORN__
FD_FN_CONST static inline ulong *fd_tcache_ring_laddr(fd_tcache_t *tcache) {
  return tcache->ring;
}
FD_FN_PURE static inline ulong *fd_tcache_map_laddr(fd_tcache_t *tcache) {
  return tcache->map;
}
#else
...
fd_tcache_reset( ulong * ring,
                 ...
                 ulong   map_cnt ) {
#ifdef __SEAHORN__
  memset( ring, FD_TCACHE_TAG_NULL, depth*sizeof(ulong) );
  memset( map, FD_TCACHE_TAG_NULL, map_cnt*sizeof(ulong) );
#else
...
#ifdef __CRAB__
FD_FN_CONST static inline ulong fd_tcache_map_start(ulong tag, ulong map_cnt) {
  ulong start = nd_ulong();
  assume(start < map_cnt);
  return start;
}
FD_FN_CONST static inline ulong fd_tcache_map_next(ulong idx, ulong map_cnt) {
  ulong next = nd_ulong();
  assume(next < map_cnt);
  return next;
}
